...
> Quirk: Use `--profile`, not `--profileId`. The long-form flag is required.
...
> Quirk: The command is `tab` (singular), not `tabs`.
...
> Quirk: Use `snap`, not `snapshot`. The alias `snap` is the intended short form.
...
> Quirk: Use `screenshot` (full word), not `ss` or `shot`.
