...
test("registers exactly one todo tool, management commands, read-only board, and collapse shortcut", () => {
	const tools: any[] = [];
	...
	assert.equal(commands[2]?.options.description, "Inspect tasks in a live read-only board");
