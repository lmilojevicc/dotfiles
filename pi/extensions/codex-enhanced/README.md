...
**Upgrading from the old in-memory reset implementation:** finish/reconcile manual requests before upgrading and stop older extension instances before enabling automation. If `/reload` detects any legacy in-process reset entry, startup saves a one-way `legacyBlocked` marker with the available original IDs (without replaying or taking over the old request). Both manual and automatic spending then remain paused, including after restart. This version intentionally provides no migration/unlock UI for legacy uncertainty: reconcile the original request with the original client and investigate the saved state; never restart or delete the journal to erase an unknown outcome. Separate older processes cannot be detected or coordinated.
