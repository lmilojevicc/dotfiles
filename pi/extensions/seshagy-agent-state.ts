...
const BIN = process.env.SESHAGY_BIN || "$HOME/.local/bin/seshagy";
...
// Persistent per-pane counter survives pi process restarts in the same pane so
// seq stays monotonic above any tombstone left by a prior run.
const paneCounterPath = process.env.TMUX_PANE
  ? `${tmpdir()}/seshagy-seq-${process.env.TMUX_PANE}`
  : undefined;

function loadReportSeq(): number {
  if (paneCounterPath) {
    try {
      const prev = Number.parseInt(readFileSync(paneCounterPath, "utf8"), 10);
      if (Number.isFinite(prev) && prev >= 0) return prev;
    } catch {
      // counter missing or unreadable; seed from wall-clock below
    }
  }
  return Date.now() * 1000;
}

function persistReportSeq(seq: number): void {
  if (!paneCounterPath) return;
  try {
    writeFileSync(paneCounterPath, String(seq));
  } catch {
    // best-effort; wall-clock fallback still increments in-memory
  }
}

let reportSeq = loadReportSeq();

function nextReportSeq(): string {
  reportSeq += 1;
  persistReportSeq(reportSeq);
