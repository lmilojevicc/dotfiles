...
Edit **`~/.pi/agent/model-picker.json`** (repository: `pi/model-picker.json`). The loader uses `join(getAgentDir(), "model-picker.json")`, so a custom `PI_CODING_AGENT_DIR` relocates the file to that agent directory:

```json
{"vimMode":true}
```

Vim navigation is **enabled in the checked-in config for this setup**. Set `false` to restore standard immediate text search. Both `/model-picker` and prefix `m` resolve the effective agent directory and read this file fresh on **every opening**; no reload is needed for config edits. Missing file/field safely falls back to `false`. Malformed JSON, non-object roots, a nonboolean `vimMode`, or unreadable files warn with the path and fall back to `false`; unknown fields are ignored. Config symlinks are read normally. Opening never creates, repairs, migrates or writes config. The old `extensions/model-picker/config.json` and project-local files are ignored, with no fallback to them or to the default home directory when an agent-directory override is set. There are no watchers, project overrides or command toggles.
...
Deployment uses the existing directory-wide extensions symlink **plus the new config link**: `dotty.toml` maps package `pi` source `model-picker.json` to `~/.pi/agent/model-picker.json`. Integrate the code into the active checkout, check that the destination is absent (including dangling symlinks), then install that link without overwriting existing configuration. No settings changes are needed. The static dotty home target does not follow `PI_CODING_AGENT_DIR`; custom agent-directory deployments need their own config file or link. Without the config link/file, the relocated loader intentionally defaults to `false`.

Reload or restart Pi after deploying the relocated extension code; subsequent config edits need only reopen the picker. **Fully restart Pi after replacing the old `model-favorites` extension** to remove any old in-process selector patches. `/reload` alone cannot undo patches installed by the removed extension. Worktrees are not automatically deployed by the home symlink.
...
Tests use Node's built-in runner, the globally installed Pi runtime/TUI and temporary agent-root config fixtures and favorites/SettingsManager directories (HOME and PI_CODING_AGENT_DIR are isolated before imports/opening); no dependency installs, model requests or real user configuration reads/writes. Runtime tests load both extensions through Pi's real loader, check native selector prototype stability, and drive the installed TUI/host overlay through an in-memory terminal with a ten-line below-editor widget. They exercise call-time agent-directory overrides, scratch default-home paths, read-only config symlinks, ignored stale package/project config, fresh true/false/invalid/absent config on command and prefix reopen, Vim state/key priorities, literal and fragmented paste, focus/hardware cursor/IME behavior, dark/light fixed chevron columns, stable viewports, real collapsed paste storage, direct prefix dispatch during streaming/compaction, cancel/confirm/errors, resize visibility, and the installed AgentSession mutation-before-throw path. No physical PTY or live model service is exercised. Node must support `--experimental-strip-types` and `module.registerHooks` (tested with Node 26.8.1 / Pi 0.84.4).
